...
export function Orb({level,active}:{level:number;active:boolean}){
 ...
 useEffect(()=>{const c=canvas.current!;const ctx=c.getContext('2d')!;let frame=0;let last=0;let rotation=0;
  ...
  const edges:number[][]=[];
  for(let i=0;i<points.length;i++)for(let j=i+1;j<points.length;j++)if(Math.hypot(...points[i].map((v,k)=>v-points[j][k]))<.29)edges.push([i,j]);
  const reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const draw=(time:number)=>{const dt=Math.min(40,time-last);last=time;if(!reduced)rotation+=dt*.00006;
   const width=c.clientWidth,height=c.clientHeight,dpr=Math.min(devicePixelRatio,2);if(c.width!==Math.round(width*dpr)||c.height!==Math.round(height*dpr)){c.width=width*dpr;c.height=height*dpr;}ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,width,height);
   const cx=width/2,cy=height/2,r=Math.min(width*.31,height*.4,135)*(1+signal.current.level*.04);
   ...
   for(const [i,j] of edges){const p=projected[i],q=projected[j];ctx.strokeStyle=`rgba(10,160,112,${.04+(p.z+1)*.09})`;ctx.lineWidth=.6;ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.stroke();}
